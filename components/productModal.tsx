...
import { fetcherPut } from '../utils/fetcher';

export default function ProductModal({ editing, editingProduct }: any) {
  ...
  function submit() {
    fetcherPut(`products/${editingProduct._id}`, { title }).then((res: any) => {
      const { status } = res;
      if (status === 200) {
